...
def train(model, device, loader, optimizer, task_type, grad_clip=0.):
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            loss = aucm_criterion(pred.to(torch.float32)[is_labeled].reshape(-1, 1), batch.y.to(torch.float32)[is_labeled].reshape(-1, 1))
            loss.backward()
...
def main():
    ...
    args.imratio = float((train_loader.dataset.data.y.sum()/train_loader.dataset.data.y.shape[0]).numpy())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_result > results['highest_valid'] and epoch > 200:
            ...
            save_ckpt(model, optimizer,
                      ...
                      sub_dir, name_post='valid_best_AUC_E_%s_R%s'%(epoch, args.random_seed))
